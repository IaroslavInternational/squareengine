	...
	dataPath(dataPath)
{
	...
	for (json::iterator m = j.begin(); m != j.end(); ++m)
	{
		...
		for (const auto& obj : j.at(d))
		{
			...
			/* Получение настроек границ мира */

			POS_X_LIMIT = obj.at("lim-x");
			POS_Y_LIMIT = obj.at("lim-y");
...
void Graphics::DrawLine(Physics::Line line, Color c)
{
	DrawLine(line.GetStartPoint(), line.GetEndPoint(), c);
}
